...
function generate_stat_grid_table() {

    console.log('Generating Stat Grid Table')
    var table = $('<table></table>')
    table.attr("id", "results")
    table.append($('<caption></caption>').text('Stats Grids'))
...
function add_challenges_to_table(table, challenge_results_type, data) {
  // console.log(data)
   ...
   data.challenge_results[challenge_results_type].forEach(function (challenge) {
       ...
       } else if (challenge_results_type == "stats_results") {
           generate_stats_table_entry(challenge, table, data)
       } else {
...
function get_grid_header_row(challenge, data) {
    ...
    //Rows 2 to 7 = Name
    main_row.append($('<th></th>').attr('colspan', 6).append(challenge.name + ' ' + help))
    //Rows 8 to 10 = Show Map
	main_row.append($('<th></th>').attr('colspan', 3).append(challenge_map_link))
	//Rows 11 to 12 = Completed Date
    main_row.append($('<th></th>').attr('colspan', 2).text(challenge.completed_on))
    //Rows 13 to 14 = Summary Text
	if (challenge.summary_text !== undefined) {
        main_row.append($('<th colspan="2" text-align="right">'+challenge.summary_text+'</th>'))
    } else {
      if (challenge.subparts_completed_count !== undefined && challenge.subparts_count !== undefined){
        ...
        main_row.append($('<th colspan="2" style="text-align:right">'+progress+'</th>'))
      }
    }
	//Row 15 Completed Tick
...
function generate_stats_table_entry(challenge, table, data) {
    ...
    // Create the header row and add it to the tbody that exists to hold
    // the title row
    var main_row = get_grid_header_row(challenge, data)
    challenge_tbody_header.append(main_row)
    ...
    // Print the subparts
    for (let i = 0; i < challenge.subparts_detail.length; i++)
	{//forEach(function (subpart_detail) {
        ...
        if (subpart_detail != null) {
			if(subpart_detail.name != null)
			{
				c = '; background-color:#90ee90"'
			    var eventlink = subpart_detail.info.match(/href="([^"]*)/)[1]
			    eventlink = '<a href="'+eventlink+'">'+subpart_detail.subpart+'</a>'
			}
            var tablecell = $('<td align="center" style="width:6%'+c+'>'+eventlink+'</td>')
			subpart_row.append(tablecell)
