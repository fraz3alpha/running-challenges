  ...
  $("table[id=results]", result).each(function (results_table_index) {
    ...
    $("h3#volunteer-summary", parent).each(function (index) {
      ...
      $("tbody>tr", results_table).each(function (role_index) {
        ...
        const normalised_volunteer_role = get_normalised_volunteer_role(volunteer_role)
...
function create_page() {
  ...
  requestSavedUserData().then((items) => {
    loaded_user_data = handleSavedUserDataResults(items)

    return requestGeoData();
  }).then((results) => {
    loaded_geo_data = handleGeoDataResults(results)

    return requestVolunteerData()
  }).then((results) => {
    parsed_volunteer_data = handleVolunteerDataResults(results);
    set_progress_message(`Parsed volunteer data ${JSON.stringify(parsed_volunteer_data, null, 2)}`)
    ...
    set_progress_message('Almost done!')
    ...
    set_progress_message(`Processed all data ${JSON.stringify(data, null, 2)}`)
  ...
  }).catch(error => handleLoadError(error, data));

}

function handleSavedUserDataResults(items) {
  set_progress_message(`Loaded saved data: ${JSON.stringify(items, null, 2)}`)
  return items
}

function handleVolunteerDataResults(results) {
...
function handleGeoDataResults(results) {
  set_progress_message(`Loaded geo data ${JSON.stringify(results, undefined, 2)}`);

  // The return packet will normally be valid even if the geo data is not contained
  // within, so we do some sanity check here
  if (!results?.geo?.data?.valid) {
    set_progress_message("Geo data rejected")
  }
  return results.geo
}

function handleLoadError(error, data) {
  console.log(error)
